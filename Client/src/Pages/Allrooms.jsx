    ...
    <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
      ...
      <div className='w-full'>
        ...
        {roomsDummyData.map((room) => (
          <div key={room._id} className='flex flex-col md:flex-row items-start gap-4 mb-8 bg-white rounded-xl shadow-md p-4 hover:shadow-lg transition-all'>
            ...
            <div className='md:w-1/2 flex flex-col justify-center gap-2'>
              <p className='text-gray-500 text-sm font-medium uppercase'>{room.hotel.city}</p>
              
              <p 
                onClick={() => { navigate(`/rooms/${room._id}`); scrollTo(0, 0) }} 
                className='text-gray-800 text-2xl md:text-3xl font-playfair cursor-pointer hover:text-indigo-600 transition-colors'
              >
                {room.hotel.name}
              </p>
              ...
              <div className='flex flex-wrap items-center mt-3 mb-6 gap-4'>
                {room.amenities.map((items, index) =>(<div key={index} className='flex items-center gap-2 px-3 py-2 rounded-lg bg-[#F5F5FF]/70'>
